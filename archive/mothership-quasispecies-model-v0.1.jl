...
md"""
# mothership-quasispecies-model-v0.1.jl 
...
Small multicopy plasmids can really amplify ARG expression, but in nature, ARGs tend to be enriched on large conjugative plasmids and not small multicopy plasmids. A open question therefore is whether small multicopy plasmids significantly contribute to resistance evolution in nature and in the clinic.
...
Laboratory experiments show dramatic ARG amplificatons due to ARG-TE transpositions onto small high-copy plasmids (Yi's paper, Yuanchi's paper, my darwinian circuit paper). This is beneficial because immediately leads to high-level resistance.
...
## 2) high plasmid copy number and balancing selection allows persistence of the streamlined plasmid.
...
## 3) a small positive ARG deletion rate allows persistence of the streamlined plasmid. (note that the population size here is "infinite", so timescale for emergence of the streamlined plasmid is determined by the mutation supply rate N*δ where N is the population size and δ is the ARG deletion rate.)
...
PlasmidSelectionCoeffSlider = @bind PLASMID_SELECTION_COEF Slider(-1.0:0.01:1.0, default=0, show_value=true)
...
PlasmidStreamlingRateSlider = @bind ARG_DELETION_RATE Slider(0:1e-8:1e-6, default=0, show_value=true)
...
function CalcNormalizedTopEigenvector(pcn, s, δ, tet_conc)
	## max transposon copy number is tied to plasmid_copy_number
	max_tcn = pcn + 1
	
	A = MutSelMatrix(pcn, s, δ, tet_conc)
