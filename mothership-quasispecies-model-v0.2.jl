...
md"""
# mothership-quasispecies-model-v0.2.jl 

by Rohan Maddamsetti  

Julia version 1.11.1.  

## Mothership hypothesis for why ARG-TEs are rarely found on small plasmids

##### Abstract

Small multicopy plasmids can dramatically amplify ARG expression, but in nature, ARGs tend to be enriched on large conjugative plasmids and not small multicopy plasmids. A open question therefore is whether small multicopy plasmids significantly contribute to resistance evolution in nature and in the clinic.

Two possible explanations:

1) *null hypothesis*: small multicopy plasmids play little to no role in bacterial adaptation (selfish intragenomic "commensals"). This means that the large body of research examining dynamics of antibiotic resistance evolution on small plasmids may have little relevance for clinical dynamics (TODO: detailed literature review on these experimental papers).  


2) *Mothership hypothesis*: Perhaps small multicopy plasmids can serve as temporary "landing pads" to amplify ARGs when needed, while the big conjugative plasmids serve as longer-term repositories for ARGs and other mobile genes in the environment.

"""

# ╔═╡ 0d79173c-7a98-41ca-a834-dadad4498c49
md""" ## Mothership hypothesis notes

Laboratory experiments show dramatic ARG amplificatons due to ARG-TE transpositions onto small high-copy plasmids (Yi's paper, Yuanchi's paper, my darwinian circuit paper). This is beneficial because immediately leads to high-level resistance. Figure 1


However, ARGs are almost always found on large conjugative plasmids, and NOT small multicopy plasmids in nature (FIGURE 1: MAKE A BIOINFORMATICS FIGURE TO ILLUSTRATE THIS, AND A FIGURE TO ILLUSTRATE THIS PARADOX MORE GENERALLY).

*Mothership hypothesis*: ARGs are stably maintained and transferred on big plasmids. ARGs are *transiently* amplified on small plasmids. ARGs on small plasmids *decrease* plasmid fitness due to transcription-replication conflict/tradeoff.

*Subhypothesis*: ARGs preferentially jump to small plasmids:
1) lower cost (no essential genes)
2) lower supercoiling (DNA is more accessible).
To justify these claims, can cite Tal Dagan/Tarnita Wein's Nat Comms paper on emergence of plasmid stability. IN ADDITION-- repeat my back of the envelope calculation showing that the enrichment of ARG-transpositions on small plasmids is higher than one would expect based on a null hypothesis of random transposition based solely on target size of chromosomes versus plasmids (FIGURE 1 subpanel or FIGURE 2: MAKE A FIGURE TO ILLUSTRATE THIS POINT).



"""

# ╔═╡ 980b266a-bad6-4e40-b52a-af1fd06c7047


# ╔═╡ e097d454-eaee-4bdb-8f35-f72b9faa55a9
md"""
### NIH grant notes:

Aim 1: test hypothesis/mechanism: small plasmids are a transient platform for ARG/gene amplification. 

Test generality with different kinds of genes (beyond ARGs) and strains. Copper resistance and Zinc metabolism? Work with strains/plasmids from Jeff Boyd and Zach? 1) work with lab strains 2) work with clinical strains in grant.

Aim 2: bridge to application: Can hyperactive Tn5 destabilize small plasmids? Can this accelerate the evolution of collateral sensitivity, by using synthetic transposons and delivery? (collateral sensitivity in this case is selection for rapid resistance that ALSO introduces some big sensitivity to some other drug or treatment).

Aim 3: test of application: see if we can drive collateral sensivity in some kind of applied context.
...
## 1) intracellular plasmid selection allows persistence of the streamlined plasmid.

### **FALSE.**

Try sₚ = -0.2, PCN = 25, [Tet] = 25.

When there is a subpopulation of cells with small plasmids saturated with transposons, then there are NO intracellular streamlined plasmids to compete with. Under antibiotic selection, the saturated subpopulation goes to fixation. Rather, intracellular plasmid competition delays the establishment of the saturated subpopulation.

## 2) a small positive ARG deletion rate allows persistence of the streamlined plasmid. (since a continuous population has an "infinite" population size, we model a finite population by rounding down the frequency of mutants under a 10-7 threshold.) timescale for emergence of the streamlined plasmid is determined by the mutation supply rate N*δ where N is the population size and δ is the per capita ARG deletion rate.)

### **TRUE.**

Try sₚ = 0, PCN = 25, [Tet] = 25, δ = 0.000001

The small ARG deletion rate means that saturated plasmid state is no longer an absorbing state. Note that the given model simulation assumes that in absence of antibiotic, ARG amplifications are costly to the host cell.

## 3) high plasmid copy number and balancing selection allows persistence of the streamlined plasmid.

### **TRUE.**

Try sₚ = 0, PCN = 50, [Tet] = 25.

This follows from my paper "emergence of population-level feedback control by transposon-plasmid coevolution". If sufficiently costly, then the antibiotic plasmid is buffered by the empty plasmid within single cells.
...
I built a mathematical model to study the mothership hypothesis.
...
md""" ### notes on intracellular plasmid selection dynamics """

# ╔═╡ dbdabe91-679f-48c9-95b5-665a08e444a7
load("../data/WF-selection-definition.png")
...
function ZeroBasedBinomialSwitchingEntry(a, b, plasmid_copy_num, sₚ)
	""" Input parameters:
	    a: number of tetA transposons on plasmids in offspring.
	    b: number of tetA transposons on plasmids in parent.
	    plasmid_copy_num: total plasmid copy number.
		sₚ: plasmid selection coefficient.
	"""
	plasmid_copy_num_after_plasmid_competition = b*(1+sₚ) + plasmid_copy_num - b
	## probability of sampling a plasmid with the tetA transposon, after one round of intracellular plasmid replication.
	## See notes on Wright-Fisher model with selection.
	p = b*(1+sₚ)/plasmid_copy_num_after_plasmid_competition

	## handle potential out-of-bounds floating-point error:
	if isapprox(p, 1.0)
		p = 1.0
	elseif isapprox(p, 0.0)
		p = 0.0
	end
...
function OneBasedBinomialSwitchingEntry(i, j, plasmid_copy_num, sₚ)
	""" Input parameters:
	    i: row-index of binomial switching matrix: 1 + number of tetA transposons on plasmids in offspring.
	    j: column-index of binomial switching matrix: 1 + number of tetA transposons on plasmids in parent.
	    plasmid_copy_num: plasmid copy number.
		sₚ: plasmid selection coefficient.

	Use ZeroBasedBinomialSwitchingEntry() with this change-of-variables:
	i = a + 1 ## a is number of tetA transposons on plasmids in offspring.
	j = b + 1 ## b is number of tetA transposons on plasmids in parent.
	
	"""
	return ZeroBasedBinomialSwitchingEntry(i-1,j-1,plasmid_copy_num, sₚ)
end
...
begin
	...
	default(fontfamily="DejaVu Sans", ##"Arial", 
		tickfontsize=12,
		xlabelfontsize=9,
		ylabelfontsize=9,
		legendfontsize=6,
...
PCNSlider = @bind PCN Slider(1:20, default=10, show_value=true)

# ╔═╡ a506ff86-41c6-44ac-adf5-c3fdb368cb02
PlasmidSelectionCoeffSlider = @bind PLASMID_SELECTION_COEF Slider(-1.0:0.01:1.0, default=-0.05, show_value=true)

# ╔═╡ 0cf84c01-0378-44be-b878-3c2119b468b9
function BinomialTransitionMatrix(plasmid_copy_num=PCN, sₚ=PLASMID_SELECTION_COEF)
	...
	for i in 1:max_TCN
		for j in 1:max_TCN
			if (i <= max_TCN) && (j <= max_TCN)
				transition_matrix[i,j] = OneBasedBinomialSwitchingEntry(i,j,plasmid_copy_num, sₚ)
			end
		end
	end
...
# ╔═╡ d981104c-7fe5-493d-9d54-aab35409cd2d
PlasmidStreamliningRateSlider = @bind ARG_DELETION_RATE Slider(0:1e-8:1e-6, default=1e-7, show_value=true)

# ╔═╡ 260df35d-6d1c-42be-8270-01fb62627938
function TransposonSwitchingMatrix(plasmid_copy_num=PCN, sₚ=PLASMID_SELECTION_COEF, δₚ=ARG_DELETION_RATE, η=η₀)
	""" Take the binomial transition matrix, and add 
	transposon jumping from chromosome to plasmid."""
	switching_matrix = BinomialTransitionMatrix(plasmid_copy_num, sₚ)
	...
	## now add in spontaneous deletions of the ARG from the plasmid.
	## when δₚ == 0, saturated plasmids is an absorbing state.
	switching_matrix[max_TCN - 1,max_TCN] = δₚ
	switching_matrix[max_TCN, max_TCN] = 1 - δₚ
	return(switching_matrix)
end

# ╔═╡ 6b2aadd7-b2c7-4928-b800-6b578a93786d
TetConcSlider = @bind TET_CONC Slider(0:20, default=10, show_value=true)
...
function MutSelMatrix(plasmid_copy_num=PCN, sₚ=PLASMID_SELECTION_COEF, δₚ=ARG_DELETION_RATE, Tet_conc=TET_CONC)
	mutsel_matrix = TransposonSwitchingMatrix(plasmid_copy_num, sₚ, δₚ) * SelectionDiagonalMatrix(plasmid_copy_num, Tet_conc)
...
function quasispecies_odefunc(du, u, p, t; ϵ=1e-7)
	...
	A = MutSelMatrix(pcn, s, δ, cur_tet_conc)
...
function SolveTetQuasispeciesSystem(my_initial_pop_vec, my_pcn, my_s, my_δ, my_tet_function;
	my_tspan=tspan)
	""" 
	...
	prob = ODEProblem(quasispecies_odefunc, my_initial_pop_vec, my_tspan, (my_pcn, my_s, my_δ, my_tet_function))
...
function SolvePulseTetQuasispeciesSystem(my_initial_pop_vec, my_pcn, my_s, my_δ, my_tet_conc;
	my_tspan=tspan)
	## provide pulses of [Tet].
	my_tet_function = CreateTetPulseFunction(my_tet_conc)
	return SolveTetQuasispeciesSystem(my_initial_pop_vec, my_pcn, my_s, my_δ, my_tet_function; my_tspan)
end
...
function SolveConstantTetQuasispeciesSystem(my_initial_pop_vec, my_pcn, my_s, my_δ, my_tet_conc;
	my_tspan=tspan)
...
begin
	...
	sol = SolveConstantTetQuasispeciesSystem(initial_pop_vec, PCN, PLASMID_SELECTION_COEF, ARG_DELETION_RATE, TET_CONC)
...
md""" ## model pulses of [Tet] in environment."""

# ╔═╡ 55f2289b-9268-4bca-9f0f-7f27e29697f8
begin 
	## run the simulation for pulses of [Tet] concentration.	
	pulse_sol = SolvePulseTetQuasispeciesSystem(initial_pop_vec, PCN, PLASMID_SELECTION_COEF, ARG_DELETION_RATE, TET_CONC)
...
md""" ### Figure 4C. plot the antibiotic pulse regime over time."""

# ╔═╡ e22f600c-4468-47ff-93f5-ad004ff5ea23
let 
	## The antibiotic pulse plot is Figure 4C.
	tet_pulse_vec = [TetPulseFunction(t) for t in 1:TIMESPAN]
	
	fig4C = plot(1:TIMESPAN, tet_pulse_vec, xlabel="time", ylabel="Antibiotic concentration", label="", size=(3.5*72,3*72))
	savefig(fig4C, "../results/Fig4C-antibiotic-pulse.pdf")
	fig4C
end

# ╔═╡ d20f047d-68d3-44eb-a258-3642d1197a88
md""" ### plot mean copy number in the population modeled by pulse_sol."""
...
# ╔═╡ 0ede4d0b-4ca7-42b6-8e6c-cb61e3578310
md""" ### Figure 4DEFG. Plot the dynamics, varying ARG deletion rates."""

# ╔═╡ e24c2c65-d2f1-4a20-a28f-75d1ffdeea77
md""" ### ERROR: LOOKS LIKE BUGGY BEHAVIOR IN PULSES WHEN δ=0 compared to log(δ) = -8 when pcn = 10, tet_conc = 10, and plasmid_selection_coef = -0.05??"""

# ╔═╡ 71d13ef2-0cd7-47de-9b4c-863d01305a80
let 
	""" Vary antibiotic deletion rates and plasmid selection coefficients in Figure 4DEF
	4GHI
	4JKL.
	"""
	pcn = 10
	plasmid_selection_coefs = [-2e-3, -2e-2, -2e-1]
	arg_deletion_rates = [1e-8, 1e-7, 1e-6] 
	tet_conc = 10
	
	pulse_sol1 = SolvePulseTetQuasispeciesSystem(initial_pop_vec, pcn, plasmid_selection_coefs[1], arg_deletion_rates[1], tet_conc)

	pulse_sol2 = SolvePulseTetQuasispeciesSystem(initial_pop_vec, pcn, plasmid_selection_coefs[1], arg_deletion_rates[2], tet_conc)

	pulse_sol3 = SolvePulseTetQuasispeciesSystem(initial_pop_vec, pcn, plasmid_selection_coefs[1], arg_deletion_rates[3], tet_conc)

	pulse_sol4 = SolvePulseTetQuasispeciesSystem(initial_pop_vec, pcn, plasmid_selection_coefs[2], arg_deletion_rates[1], tet_conc)

	pulse_sol5 = SolvePulseTetQuasispeciesSystem(initial_pop_vec, pcn, plasmid_selection_coefs[2], arg_deletion_rates[2], tet_conc)

	pulse_sol6 = SolvePulseTetQuasispeciesSystem(initial_pop_vec, pcn, plasmid_selection_coefs[2], arg_deletion_rates[3], tet_conc)

	pulse_sol7 = SolvePulseTetQuasispeciesSystem(initial_pop_vec, pcn, plasmid_selection_coefs[3], arg_deletion_rates[1], tet_conc)

	pulse_sol8 = SolvePulseTetQuasispeciesSystem(initial_pop_vec, pcn, plasmid_selection_coefs[3], arg_deletion_rates[2], tet_conc)

	pulse_sol9 = SolvePulseTetQuasispeciesSystem(initial_pop_vec, pcn, plasmid_selection_coefs[3], arg_deletion_rates[3], tet_conc)


	## calculate mean copy numbers.
	pulse_mean_copy_num_vec1 = CalcMeanTetACopyNumberVec(pulse_sol1)
	pulse_mean_copy_num_vec2 = CalcMeanTetACopyNumberVec(pulse_sol2)
	pulse_mean_copy_num_vec3 = CalcMeanTetACopyNumberVec(pulse_sol3)
	pulse_mean_copy_num_vec4 = CalcMeanTetACopyNumberVec(pulse_sol4)
	pulse_mean_copy_num_vec5 = CalcMeanTetACopyNumberVec(pulse_sol5)
	pulse_mean_copy_num_vec6 = CalcMeanTetACopyNumberVec(pulse_sol6)
	pulse_mean_copy_num_vec7 = CalcMeanTetACopyNumberVec(pulse_sol7)
	pulse_mean_copy_num_vec8 = CalcMeanTetACopyNumberVec(pulse_sol8)
	pulse_mean_copy_num_vec9 = CalcMeanTetACopyNumberVec(pulse_sol9)

	label1 = "log₁₀(sₚ)=–2.7, log₁₀(δ)=–8"
	label2 = "log₁₀(sₚ)=–2.7, log₁₀(δ)=–7"
	label3 = "log₁₀(sₚ)=–2.7, log₁₀(δ)=–6"
	label4 = "log₁₀(sₚ)=–1.7, log₁₀(δ)=–8"
	label5 = "log₁₀(sₚ)=–1.7, log₁₀(δ)=–7"
	label6 = "log₁₀(sₚ)=–1.7, log₁₀(δ)=–6"
	label7 = "log₁₀(sₚ)=–0.7, log₁₀(δ)=–8"
	label8 = "log₁₀(sₚ)=–0.7, log₁₀(δ)=–7"
	label9 = "log₁₀(sₚ)=–0.7, log₁₀(δ)=–6"
	
	## make the plots.
	fig4D = plot(xlabel="time", ylabel="Mean tetA copy number", pulse_sol1.t, pulse_mean_copy_num_vec1, label=label1)

	fig4E = plot(xlabel="time", ylabel="Mean tetA copy number", pulse_sol2.t, pulse_mean_copy_num_vec2, label=label2)
	
	fig4F = plot(xlabel="time", ylabel="Mean tetA copy number", pulse_sol3.t, pulse_mean_copy_num_vec3, label=label3)

	fig4G = plot(xlabel="time", ylabel="Mean tetA copy number", pulse_sol4.t, pulse_mean_copy_num_vec4, label=label4)

	fig4H = plot(xlabel="time", ylabel="Mean tetA copy number", pulse_sol5.t, pulse_mean_copy_num_vec5, label=label5)

	fig4I = plot(xlabel="time", ylabel="Mean tetA copy number", pulse_sol6.t, pulse_mean_copy_num_vec6, label=label6)

	fig4J = plot(xlabel="time", ylabel="Mean tetA copy number", pulse_sol7.t, pulse_mean_copy_num_vec7, label=label7)

	fig4K = plot(xlabel="time", ylabel="Mean tetA copy number", pulse_sol8.t, pulse_mean_copy_num_vec8, label=label8)

	fig4L = plot(xlabel="time", ylabel="Mean tetA copy number", pulse_sol9.t, pulse_mean_copy_num_vec9, label=label9)

	savefig(fig4D, "../results/Fig4D.pdf")
	savefig(fig4E, "../results/Fig4E.pdf")
	savefig(fig4F, "../results/Fig4F.pdf")
	savefig(fig4G, "../results/Fig4G.pdf")
	savefig(fig4H, "../results/Fig4H.pdf")
	savefig(fig4I, "../results/Fig4I.pdf")
	savefig(fig4J, "../results/Fig4J.pdf")
	savefig(fig4K, "../results/Fig4K.pdf")
	savefig(fig4L, "../results/Fig4L.pdf")
...
manifest_format = "2.0"
project_hash = "9acaada051a4eb49663c04a70b72cfc8364c5e2a"
