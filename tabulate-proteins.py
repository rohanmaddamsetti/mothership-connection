...
## make a unique SeqIndex for each tabulated sequence.
## This is incremented in the innermost loop.
seq_index = 1
with open(outf, 'w') as outfh:
    header = "SeqIndex,AnnotationAccession,count,chromosome_count,plasmid_count,unassembled_count,product,sequence\n"
    outfh.write(header)
    for gbk in tqdm(os.listdir("../data/Maddamsetti2025-results/gbk-annotation")):
        ...
        with gzip.open(infile, "rt") as genome_fh:
            ...
            for replicon in SeqIO.parse(genome_fh, "gb"):
                ...
                for feat in replicon.features:
                    ...
                    if prot_seq not in protein_dict:
                        protein_dict[prot_seq] = { "SeqIndex":str(seq_index),
                                                   "count":0,
                                                   "chromosome_count":0,
                                                   "plasmid_count":0,
                                                   "unassembled_count":0,
                                                   ...
                                                   "locations":[]}
                        seq_index += 1 ## and increment the seq_index for the next new sequence added to the dict.
            ...
            for seq, v in filtered_prot_dict.items():
                row = ','.join([v["SeqIndex"], annotation_accession, str(v["count"]), str(v["chromosome_count"]), str(v["plasmid_count"]), str(v["unassembled_count"]), v["product"], seq])
                row = row + '\n'
                outfh.write(row)
